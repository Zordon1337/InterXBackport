...
bool __stdcall CreateMoveClient_Hooked(float frametime, CUserCmd* pCmd)
{
	/*if (Interfaces::Engine->IsInGame() && Interfaces::Engine->IsConnected()) {
		static auto SetClanTag = reinterpret_cast<void(__fastcall*)(const char*, const char*)>(reinterpret_cast<uintptr_t>(GetModuleHandleA("engine.dll")) + 0x9cd40);
		...
		if (Delay) {
			if (GetTickCount() - Delay > 850) {
				...
				if (Interfaces::Engine->GetPlayerInfo(Interfaces::Engine->GetLocalPlayer(), &PlayerInfo)) {
					char Buffer[128];
					sprintf_s(Buffer, "say [CLOWN'S HOOK] %s - %i kill(s)", PlayerInfo.name, Kills2);
...
bool __fastcall Hooked_FireEventClientSide(PVOID ECX, PVOID EDX, IGameEvent *Event) {
	if (Menu::Window.MiscTab.OtherEventSpam.GetState()) {
		if (strcmp(Event->GetName(), "round_end") == 0) {
			...
			char Buffer[128];
			sprintf_s(Buffer, "say [CLOWN'S HOOK]: Hello :)");
			Interfaces::Engine->ClientCmd(Buffer);
		}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo)) {
				char Buffer[128];
				sprintf_s(Buffer, "say [CLOWN'S HOOK] %s just connected", PlayerInfo.name);
				Interfaces::Engine->ClientCmd(Buffer);
			}
		}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo) &&
				...
				if (PlayerID != Interfaces::Engine->GetLocalPlayer() && AttackerID == Interfaces::Engine->GetLocalPlayer()) {
					...
					switch (Kills) {
					case 1: sprintf_s(Buffer, "say [CLOWN'S HOOK] First kill! %s just got owned [KS: 1]", PlayerInfo.name); break;
					case 2:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Double kill! %s just got owned [KS: 2]", PlayerInfo.name); break;
					case 3:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Multi kill! %s just got owned [KS: 3]", PlayerInfo.name); break;
					case 4:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Ultra kill! %s just got owned [KS: 4]", PlayerInfo.name); break;
					case 5:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Fantastic! %s just got owned [KS: 5]", PlayerInfo.name); break;
					case 6:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Unbelievable! %s just got owned [KS: 6]", PlayerInfo.name); break;
					case 7:	sprintf_s(Buffer, "say [CLOWN'S HOOK] Unbelievable++ %s just got owned [KS: 7]", PlayerInfo.name); break;
					case 9: sprintf_s(Buffer, "say [CLOWN'S HOOK] KILLING SPREE! %s just got owned [KS: 9]", PlayerInfo.name); break;
					case 15: sprintf_s(Buffer, "say [CLOWN'S HOOK] RAMPAGE! %s just got owned [KS: 15]", PlayerInfo.name); break;
					case 21: sprintf_s(Buffer, "say [CLOWN'S HOOK] DOMINATING! %s just got owned [KS: 21]", PlayerInfo.name); break;
					case 26: sprintf_s(Buffer, "say [CLOWN'S HOOK] UNSTOPPABLE! %s just got owned [KS: 26]", PlayerInfo.name); break;
					case 37: sprintf_s(Buffer, "say [CLOWN'S HOOK] GODLIKE! %s just got owned [KS: 37]", PlayerInfo.name); break;
					case 40: sprintf_s(Buffer, "say [CLOWN'S HOOK] BEYOND GODLIKE! %s just got owned [KS: 40]", PlayerInfo.name); break;
					default: sprintf_s(Buffer, "say [CLOWN'S HOOK] %s just got owned [KS: %i]", PlayerInfo.name, Kills); break;
					}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo) &&
				...
				char Buffer[128];
				sprintf_s(Buffer, "say %s just hurt %s", AttackerInfo.name, PlayerInfo.name);
				Interfaces::Engine->ClientCmd(Buffer);
			}
		}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo)) {
				char Buffer[128];
				sprintf_s(Buffer, "say [CLOWN'S HOOK] %s just ragequit :>", PlayerInfo.name);
				Interfaces::Engine->ClientCmd(Buffer);
			}
		}
	}
...
void  __stdcall Hooked_FrameStageNotify(ClientFrameStage_t curStage)
{
	...
	if (Interfaces::Engine->IsInGame() && curStage == FRAME_RENDER_START && Menu::Window.MiscTab.OtherNoVisualRecoil.GetState())
	{
		IClientEntity *pLocal = Interfaces::EntList->GetClientEntity(Interfaces::Engine->GetLocalPlayer());

		if (pLocal && pLocal->IsAlive())
		{
			AimPunch = (Vector*)((DWORD)pLocal + 0x3018);
			ViewPunch = (Vector*)((DWORD)pLocal + 0x300C);
	...
	if (AimPunch && ViewPunch && Menu::Window.MiscTab.OtherNoVisualRecoil.GetState())
	{
