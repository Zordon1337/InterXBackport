...
void CEsp::DrawHealth(IClientEntity* pEntity, CEsp::ESPBox size)
{
	...
	Verts[1].Init(Vector2D(HealthBar.x + HealthBar.w, HealthBar.y + 4));
	Verts[2].Init(Vector2D(HealthBar.x + HealthBar.w, HealthBar.y + 4));
	Verts[3].Init(Vector2D(HealthBar.x, HealthBar.y + 4));

	Render::PolygonOutline(4, Verts, Color(0, 0, 0, 0), Color(0, 0, 0, 0));

	Vertex_t Verts2[4];
	Verts2[0].Init(Vector2D(HealthBar.x, HealthBar.y));
	Verts2[1].Init(Vector2D(HealthBar.x + HealthBar.w + 2, HealthBar.y + 4));
	Verts2[2].Init(Vector2D(HealthBar.x + HealthBar.w + 2, HealthBar.y + 4));
	Verts2[3].Init(Vector2D(HealthBar.x, HealthBar.y + 4));

	Color c = GetPlayerColor(pEntity);
	Render::Polygon(4, Verts2, Color(0, 255, 0, 255));

	Verts2[0].Init(Vector2D(HealthBar.x, HealthBar.y));
	Verts2[1].Init(Vector2D(HealthBar.x + HealthBar.w + 2, HealthBar.y)); // Player health based on color of the players team.
	Verts2[2].Init(Vector2D(HealthBar.x + HealthBar.w + 2, HealthBar.y + 4));
	Verts2[3].Init(Vector2D(HealthBar.x, HealthBar.y + 4));
...
void CEsp::DrawGlow()
{
	...
	for (int i = 0; i < GlowObjectManager->size; ++i)
	{
		...
		if (glowEntity->IsEmpty() || !Entity || IsBadCodePtr((FARPROC)Entity))
			continue;
