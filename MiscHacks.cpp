...
void CMiscHacks::Move(CUserCmd *pCmd, bool& bSendPacket)
{
	...
	if (Menu::Window.MiscTab.OtherClowns.GetState()) {
		std::string text = "";
		static bool useSpace = false;
		text = useSpace ? "™InterX " : "™CLOWN'S HOOK";
		NET_SetConVar noob("name", text.c_str());
		Interfaces::Engine->GetNetChannelInfo()->SendNetMsg(noob);
		useSpace = !useSpace;
	}
...
void CMiscHacks::ChatSpam3()
{
	...
	sprintf_s(buffer, "%s is gay xD", Names[randomIndex].c_str(), meme);
	meme += 1;
	SayInChat(buffer);
	start_t = clock();
}
...
{ "InterX 4 PRE$IDENT", "InterX - EASIEST WAY TO WIN", "InterX CO$T$ TOO MUCH", "DE$TROYED BY CLOWN'S HOOK", "InterX - EASIEST WAY TO WIN AFK","B1G CHEET C0DED 1N SIRIUSLABS", "InterX FRAMEWORK", "nice cheat do u pay for that?", "rekt by b1g cheat", "i live in a mansion do u" };
